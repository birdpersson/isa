...
public interface PharmacyEmployeeRepository extends JpaRepository<User, Long>{
	...
	@Query(value="SELECT * FROM USERS u where u.id in ( select user_id from users_work_in_pharmacies where pharmacy_id = :id) and u.work_role ='DERMATOLOGIST'", nativeQuery=true)
	List<User> getAllDermatologsByPharmacyId(@Param("id")Long id);
	@Query(value="SELECT * FROM USERS u where u.id in ( select user_id from users_work_in_pharmacies where pharmacy_id = :id) and u.work_role ='PHARMACIST'", nativeQuery=true)
	List<User> getAllPharmacistsByPharmacyId(@Param("id")Long id);
	@Query(value="SELECT * FROM USERS u where u.id in ( select user_id from users_work_in_pharmacies where pharmacy_id = :id) and u.work_role ='PHADMIN'", nativeQuery=true)
